...
for i in $(seq 1 1 3)
do
    echo "itertion: spgemm original dense-sparse marlin size compare*****">>spgemm-marlin-size-4-27th.stdout
    echo "********************************">>spgemm-marlin-size-4-27th.stdout
    for d in ${dim[@]}
    do
       nohup $SUBMIT $PARAM --name spgemm-dim-$d-858-0.01 $JAR $d $d $d 0.01 5 8 5 8 2>>stderr 1>>spgemm-marlin-size-4-27th.stdout
       sleep 20
       nohup $SUBMIT $PARAM --name spgemm-dim-$d-858-0.1  $JAR $d $d $d 0.1  5 8 5 8 2>>stderr 1>>spgemm-marlin-size-4-27th.stdout
       sleep 20
    done
done
...
for i in $(seq 1 1 2)
do
    echo "itertion: spgemm to dense-dense marlin size compare*****">>spgemm-marlin-size-4-27th.stdout
    echo "********************************">>spgemm-marlin-size-4-27th.stdout
    for d in ${dim[@]}
    do
       nohup $SUBMIT $PARAM --name spgemm-dim-$d-858-0.01 $JAR $d $d $d 0.01 6 8 5 8 2>>stderr 1>>spgemm-marlin-size-4-27th.stdout
       sleep 20
       nohup $SUBMIT $PARAM --name spgemm-dim-$d-858-0.1  $JAR $d $d $d 0.1  6 8 5 8 2>>stderr 1>>spgemm-marlin-size-4-27th.stdout
       sleep 20
    done
done
